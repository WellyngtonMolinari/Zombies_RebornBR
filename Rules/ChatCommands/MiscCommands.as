...
class TsarCommand : BlobCommand
{
	TsarCommand()
	{
		super("mikilltsarbombcomandosupersecreto", "Comando super secreto que ninguem vai descobrir do mikill!");
	}

	void SpawnBlobAt(Vec2f pos, string[] args, CPlayer@ player)
	{
		CBlob@ tsarbomb = server_CreateBlob("tsarbomb", -1, pos);
		tsarbomb.server_SetQuantity(1);
	}
}
...
class DiscordCommand : ChatCommand
{
	DiscordCommand()
	{
		super("discord", "Link do servidor de discord do Kag Brasil");
		AddAlias("dc");
	}

	void Execute(string[] args, CPlayer@ player)
	{
			OpenWebsite("https://discord.gg/TeU3zrQuuv");
	}
}
